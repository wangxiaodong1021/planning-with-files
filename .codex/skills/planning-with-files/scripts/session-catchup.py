...
def planning_root_from_env(project_path: str) -> Path:
    plan_dir = os.getenv('CODEX_PLAN_DIR', '').strip()
    if not plan_dir:
...
def main():
    ...
    planning_root = planning_root_from_env(project_path)

    # Check if planning files exist (indicates active task)
    has_planning_files = any(
        Path(planning_root, f).exists() for f in PLANNING_FILES
    )
    if not has_planning_files:
        # No planning files for the active plan; skip catchup to avoid noise.
        return
    ...
    print(f"Planning directory: {planning_root}")
    ...
    print(f"2. Read: {planning_root / 'task_plan.md'}, {planning_root / 'progress.md'}, {planning_root / 'findings.md'}")
